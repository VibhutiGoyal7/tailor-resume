...
| Build progress | **Milestone 5 (resume-engine, retrieve stage) built; on branch `feature/resume-engine-retrieve`, not yet merged.** M1–M4 merged to `dev` (`main` tagged `v0.1-milestone4-parse`). **M5 (this session):** the RAG retrieval step — `parseJD` now chains to `retrieveCandidates`, which lazily embeds kept bullets (Voyage, worker-side), runs pgvector cosine search per JD query, unions + hybrid tag-re-ranks (ADR-003), and stops at the ADR-017 checkpoint (`GET /jobs/:jobId` returns `retrievedCandidates`; `POST /jobs/:jobId/confirm` records the kept subset). Embedder behind `Embedder`; auto-falls back to a **deterministic stub when `VOYAGE_API_KEY` is unset** (real Voyage when set — no code change). pgvector raw SQL lives in the profile facade (owns the table); resume-engine stays boundary-clean. New `TailoredResume.retrievedCandidates` Json column (migration `20260727063806`). Generate-enqueue on confirm deferred to M6. Google sign-in still deferred (slice 2b, 2026-07-25). Lint/format/**repo-wide typecheck**/test green (125 tests; DB+pgvector via `RUN_DB_TESTS=1`) + live worker+Redis+pgvector smoke passed (parse→retrieve→confirm). |
