...
| Build progress | **Milestones 6–7 (resume-engine: generate + render/export + layout customization) built; on branch `feature/resume-engine-layout` (stacked on the unmerged `feature/resume-engine-generate`), not yet merged.** M1–M5 merged to `dev` (`main` tagged `v0.1-milestone4-parse`). **M6:** (a) **generate** — `POST /jobs/:jobId/confirm` advances `awaiting_confirmation → generating` and enqueues `generateResume` (the *only* place generate is enqueued — ADR-017); `runGenerateStage` calls Claude **Sonnet** (ADR-004, `claude-sonnet-5`, structured output) to select/rewrite the kept bullets, **reconciles grounding** (drops hallucinated `sourceBulletId`s, attaches real `experienceItemId` traces, dedupes, caps at 12), and advances `generating → done`. (b) **render + export** — the same stage renders **PDF** (`@react-pdf/renderer`) + **DOCX** (`docx`) for **3 templates** (`ats`/`modern`/`compact`, ADR-018) and stores them via a `FileStore`; `templateId` is auto-suggested from `company_type` at parse time (`suggestTemplateId`, ADR-012). Routes `GET /resumes/:id` (detail + `availableFormats`) and `GET /resumes/:id/export?format=pdf|docx`. **M7 (this session):** (a) **history + delete** — `GET /resumes` (`listResumes`, newest-first summaries) and `DELETE /resumes/:id` (`deleteResume` — removes export files via new idempotent `FileStore.delete`, then the linked job + row). (b) **layout customization** — `PATCH /resumes/:id/layout` (`updateResumeLayout`): reorder/hide sections, switch **layout variant** (`modern` sidebar left/right; single-column templates one canonical variant each — ADR-018 follow-up), or **override the template**; 409 until generated; validated variant. It persists + **enqueues an async re-render** on a new `render-resume` queue (`runRenderStage`) that overwrites the PDF/DOCX **in place** (worker-only renderers, CLAUDE.md §7); the renderers honor section order/hidden/variant. **All keyless:** generator stubs w/o `ANTHROPIC_API_KEY`; `FileStore` = local disk w/o `R2_*`; renderer worker-injected. **ADR-012 amended** (docx lib, storage, React-18 pin, + M7 layout-variant/re-render note). Google sign-in still deferred (slice 2b). Merged to `main`, tagged `v0.2-milestone7-resume-engine`. **M8 in progress (mobile app):** slice 1 — **foundation** — built on `feature/mobile-foundation` (not merged): React Navigation 7 shell (auth stack ↔ 4-tab bar gated by auth status), TanStack Query, one API client (bearer + canonical-error parsing + transparent 401 refresh via a React-decoupled auth bridge), `expo-secure-store` tokens (ADR-010), one `errorCopy` map + one `ErrorDialog` (CLAUDE.md §4), functional Login/Signup/Sign-out. Remaining M8 slices: 2 auth completion (verify/forgot/reset), 3 Bank, 4 tailoring flow, 5 Resumes/Profile. Backend green at 175 tests; **188 total with 18 new mobile unit tests** (pure logic: client helpers, error mapping, auth reducer). Lint/typecheck clean. |
...
**Amendment (Milestone 7, layout customization) — layout variants + an async re-render queue.** Two additions while building `updateResumeLayout`; neither changes ADR-012's or ADR-018's reasoning:
- **Layout variants (ADR-018 follow-up).** The open item "which variants each template gets" is decided minimally and honestly: `modern` (two-column) gets **sidebar left / sidebar right**; `ats` and `compact` (single-column) get **one canonical variant each**. Richer per-template variants are a later, non-blocking design pass — flagged so the small set isn't read as final. Registry in `shared-types` (`LAYOUT_VARIANTS`).
- **Async re-render on a new `render-resume` queue.** A layout change can't render on the web side (renderers are worker-only, CLAUDE.md §7), so `PATCH /resumes/:id/layout` persists the layout and enqueues a resume-scoped re-render; the worker's `renderResume` task → `runRenderStage` re-renders PDF/DOCX from the stored `renderedContent` + new layout and **overwrites the export files in place** (deterministic FileStore keys keep the download URL stable). This is a 4th queue extending the existing ADR-009 chaining pattern, not a new architecture. `RenderedResume` gained a `skills` field so a re-render needs no retrieval candidates.
