...
| Build progress | **Milestone 6 (resume-engine, generate stage) built; on branch `feature/resume-engine-generate`, not yet merged.** M1–M5 merged to `dev` (`main` tagged `v0.1-milestone4-parse`). **M6 (this session):** the generation step — `POST /jobs/:jobId/confirm` now advances `awaiting_confirmation → generating` and enqueues `generateResume` (the *only* place generate is enqueued — ADR-017 two-phase). `runGenerateStage` loads the parsed JD + the kept candidate snapshot, pulls optional Resume Basics for the summary, calls Claude **Sonnet** (ADR-004, `claude-sonnet-5`) for a **structured** bullet selection/rewrite, **reconciles grounding** (drops hallucinated `sourceBulletId`s, attaches the real `experienceItemId` per-bullet trace, dedupes, caps at 12), persists `renderedContent`, and advances `generating → done`. Generator behind a `ResumeGenerator` interface; auto-falls back to a **deterministic stub when `ANTHROPIC_API_KEY` is unset** (real Sonnet when set — no code change), so the full parse→retrieve→confirm→generate pipeline runs **keyless**. `GET /jobs/:jobId` now returns `renderedContent` once `done`. Boundary-clean (ADR-008 — reads the kept candidates from the persisted snapshot, never re-queries profile's tables). Still deferred: the react-pdf/docx **renderers** + export endpoint (ADR-012) and `templateId` selection (placeholder `"classic"`). Google sign-in still deferred (slice 2b, 2026-07-25). Lint/format/**repo-wide typecheck**/test green (142 tests; DB+pgvector via `RUN_DB_TESTS=1`) + live keyless parse→retrieve→confirm→generate→`done` flow verified against Postgres. |
...
| PDF rendering | @react-pdf/renderer (component-based, runs in worker process) | See ADR-012 |
| DOCX rendering | docxtemplater (separate path from PDF) | See ADR-012 |
| File storage | Cloudflare R2 (free tier, no egress fees) or S3 | Generated PDF resumes |
