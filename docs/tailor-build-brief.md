...
### Implementation status — Milestones 4–5 (parse + retrieve stages)

The **parse** and **retrieve** stages are implemented end-to-end (through the ADR-017 checkpoint); **generate** remains a scaffold.
...
- **Deferred (intentional):** `confirmRetrievedMatches` records the kept set but does **not** yet enqueue `generateResume` — that wiring lands in Milestone 6 with the generate task (same clean-boundary pattern parse→retrieve used), so the job stays at `awaiting_confirmation` with the selection recorded. `TailoredResume.templateId` is still a placeholder `"classic"` until template selection is designed (§10 open items).
