...
PATCH  /api/account/password              { currentPassword, newPassword } → { accessToken, refreshToken }  (revokes other sessions, rotates the caller's pair)
...
- **Customize-layout section set — RESOLVED (owner decision: grow the model).** The design (`tailor_screen_layout_customize.svg`) mocks *five* reorderable rows (Summary, Experience, Projects, Education, Skills). The content model now has all five as first-class sections: `RESUME_SECTIONS = [summary, skills, experience, projects, education]`, and each tailored bullet carries a `section` (`RenderedBullet.section`, one of experience/projects/education) assigned at generate time from the kind of its source Experience item (`sectionForItemType`: project → Projects, education → Education, role/other → Experience). Both renderers (PDF + DOCX) emit Projects/Education as their own headings and skip an empty section; the customize screen shows all five reorderable/hideable rows. No DB migration — `renderedContent` is a Json column and `section` is an added, optional field (legacy resumes without it default to Experience). The one-/two-column toggle still maps to the template's layout family (two-column → `modern`; one-column → a single-column template). Covered by generation/render/shared-types unit tests.
...
- **Resumes tab** (`ResumesStack`: list → detail). The **list** (`tailor_screen_resumes_list.svg`) reads `GET /resumes`, rendering tilted white history cards (role, "company · template name", relative time, compact match ring); the header "+" and the FAB both open the JD-input flow on the root stack; empty history shows the dog-eared-page motif. The **detail** (`tailor_screen_resume_detail.svg`) reads `GET /resumes/:id`: the tailored bullets, the per-bullet source trace (`from: …`), Edit-layout / Export (which push the existing full-screen root-stack flow screens) and a destructive Delete (`DELETE /resumes/:id`).
  - **Resume-detail EXPERIENCE / PROJECTS split:** the design groups bullets under EXPERIENCE / PROJECTS / EDUCATION headings. Now backed by the first-class `RenderedBullet.section` (see the resolved item above); `lib/resume.groupResumeBullets` groups by that field, falling back to the source item's kind (via `GET /bank`) for legacy resumes generated before `section` was persisted. The bank is still read to render each bullet's `from: …` source-trace label. Unit-tested.
- **Profile tab** (`ProfileStack`: settings → sub-screens). **Settings** (`tailor_screen_profile_settings.svg`): avatar initials + name (from Resume basics) + email (from `GET /account`), the settings rows, **Log out** (clears the session) and **Delete account** (`DELETE /account`, then clears the session). **Resume basics** (`tailor_screen_resume_basics.svg`): the contact/summary form, `GET`/`PUT /bank/basics`, backend field errors surfaced per-field. **How this works** (`tailor_screen_how_this_works.svg`): a static explainer of the parse→retrieve→generate pipeline with an illustrative match dial.
  ...
  - **Password change ends *other* sessions only (resolved).** Previously `authModule.changePassword` revoked *every* refresh token (including the caller's), contradicting §5's "revoke other sessions." Fixed per owner decision: `changePassword` now revokes all existing refresh tokens **and mints a fresh pair for the calling device, returning it** — so the device that changed the password stays signed in while all others are logged out. Contract change: `PATCH /account/password` now responds with `{ accessToken, refreshToken }` (was `{ ok: true }`); the mobile `AuthContext.changePassword` swaps to the returned pair, and the Password screen's success state reads "you're still signed in here; other devices were signed out." Covered by the facade + web route tests.
